...
# inspect if lambda_index is argument of a linear model' forward function (kwargs)
def is_lambda_index_in_forward(model):
    forward_args = inspect.getfullargspec(model.forward).args
    return 'lambda_index' in forward_args
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        # get query proj
        if is_lambda_index_in_forward(self.q_proj):
            query_states = self.q_proj(hidden_states, lambda_index) * self.scaling
        else:
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            # cross_attentions
            if is_lambda_index_in_forward(self.k_proj):
                key_states = self._shape(self.k_proj(key_value_states, lambda_index), -1, bsz)
            else:
                key_states = self._shape(self.k_proj(key_value_states), -1, bsz)
            
            if is_lambda_index_in_forward(self.v_proj):
                value_states = self._shape(self.v_proj(key_value_states, lambda_index), -1, bsz)
            else:
                value_states = self._shape(self.v_proj(key_value_states), -1, bsz)
        elif past_key_value is not None:
            # reuse k, v, self_attention
            if is_lambda_index_in_forward(self.k_proj):
                key_states = self._shape(self.k_proj(hidden_states, lambda_index), -1, bsz)
            else:
                key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            
            if is_lambda_index_in_forward(self.v_proj):
                value_states = self._shape(self.v_proj(hidden_states, lambda_index), -1, bsz)
            else:
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor, past_key_value: tuple = None, block_adapters=None, task=None, lambda_index=None, output_attentions: bool = False):
        """
        Args:
        ...
        if self.use_lora:
            hidden_states, attn_weights, _ = self.self_attn(
                hidden_states=hidden_states, attention_mask=attention_mask, past_key_value=past_key_value, output_attentions=output_attentions, task=task, lambda_index=lambda_index
            )
        else:
            hidden_states, attn_weights, _ = self.self_attn(
                hidden_states=hidden_states, attention_mask=attention_mask, past_key_value=past_key_value, output_attentions=output_attentions, lambda_index=lambda_index
            )
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False):
                    ...
                    layer_outputs = encoder_layer(hidden_states, attention_mask, past_key_value, block_adapters, task, lambda_index=lambda_index, output_attentions=output_attentions)
