...
class GQAFineTuneDataset(Dataset):
    def __init__(self, split='train,valid', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        self.args = args
        if hasattr(self.args, "lambda_index"):
            self.lambda_index = self.args.lambda_index
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.lambda_index is not None:
            out_dict['lambda_index'] = self.lambda_index
    ...
    def collate_fn(self, batch):
        ...
        batch_entry["lambda_index"] = torch.tensor([entry['lambda_index'] for entry in batch], dtype=torch.long) if 'lambda_index' in batch[0] else None
