...
class mLoRALinear(nn.Module):
    def __init__(
        ...
        self.in_features = m.in_features
        self.out_features = m.out_features
        self.weight = nn.Parameter(m.weight.detach(), requires_grad=False)
        self.bias = nn.Parameter(m.bias.detach(), requires_grad=False) if m.bias is not None else None
        ...
        self.lora_A = nn.Parameter(
            self.weight.new_zeros((1, r, self.in_features))
        )
        ...
        if not self.diagonal_format:
            ...
            self.lora_lambdas = nn.Parameter(
                self.weight.new_zeros((lambda_num, r))
            )

        self.lora_B = nn.Parameter(
            self.weight.new_zeros((B_num, self.out_features, r))
        )
        self.lora_B_w = nn.Parameter(
    ...
    def reset_parameters(self):
        if hasattr(self, "lora_A"):
            ...
            # with torch.no_grad():
                # self.lora_B_w.data[-1] = torch.tensor([1.0, 0.0, 0.0])

    def forward(self, x: torch.Tensor, lambda_index: torch.Tensor):
        def T(w):
            return w

        if lambda_index.device != self.lora_A.device:
            lambda_index = lambda_index.to(self.lora_A.device)
    ...
    def extra_repr(self) -> str:
        return 'in_features={}, out_features={}, bias={}, lora_dim={}, lora_scale={}'.format(
            self.in_features, self.out_features, self.bias is not None, self.r, self.scaling
        )
                ...
                for n, p in it:
                    ...
                    if any(t in n for t in dora_targets) and isinstance(p,nn.Linear):
                        if any(t in n for t in dora_lora_target):
                            ...
                            del p
        if self.args.use_mlora:
            print("apply mlora tuning")
            dora_targets = ["layers"]
            dora_lora_target = ["v_proj", "q_proj"]
            it=[(name,m) for name,m in self.model.named_modules()]
            module_dict={}
            for n, p in it:
                ...
                if any(t in n for t in dora_targets) and isinstance(p,nn.Linear):
                    if any(t in n for t in dora_lora_target):
                        ...
                        replace_m = mLoRALinear(m = p, 
                                                r = self.args.lora_dim, 
                                                lora_dropout = 0.0, 
                                                B_num = self.args.B_num,
                                                lambda_num = self.args.lambda_num,
                                                diagonal_format = False,
                                                B_scale = self.args.mlora_temperature,
                        ...
                        replace_m.lora_B_w.requires_grad = True
                        replace_m.lora_lambdas.requires_grad = True
                        replace_m.weight.requires_grad = False
                        replace_m.bias.requires_grad = False
                        del p
