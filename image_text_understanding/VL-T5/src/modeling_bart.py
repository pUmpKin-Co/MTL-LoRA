        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False):
                    ...
                    layer_outputs = encoder_layer(hidden_states, attention_mask, past_key_value, block_adapters, task=task, lambda_index=lambda_index, output_attentions=output_attentions, )
...
class VLBart(BartForConditionalGeneration):
    ...
    def prepare_inputs_for_generation(
        ...
        if "lambda_index" in kwargs:
            if kwargs["lambda_index"].shape[0] != decoder_input_ids.shape[0]:
                kwargs["lambda_index"] =  kwargs["lambda_index"].repeat(decoder_input_ids.shape[0] // kwargs["lambda_index"].shape[0])
            output["lambda_index"] = kwargs["lambda_index"]
